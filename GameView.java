...
public class GameView extends JFrame {
    ...
    public GameView(GameModel gameModel, GameController gameController) {
        ...
        random.addActionListener(gameController); //created all buttons

        grid = new JPanel();
        grid.setLayout(new GridLayout(model.getHeight(), model.getWidth(), 2, 2)); //created a grid JPanel

        steps = new JLabel("Number of steps: " + model.getNumberOfSteps());
        square = new GridButton[model.getHeight()][model.getWidth()];
        for (int i = 0; i<gameModel.getHeight(); i++){ //sets up each button on the grid JPanel
        ...
        buttons = new JPanel();
        buttons.setLayout(new BoxLayout(buttons, BoxLayout.PAGE_AXIS)); //create a panel for the buttons
        buttons.add(reset);
        buttons.add(random);
        ...
        stepsLayout.setLayout(new BorderLayout()); //create a panel for the steps
        ...
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); //add everything to the Frame and set it up
        setVisible(true);
